...
- `GET /api/strategies` (Phase 4.8) is a thin wrapper around `strategy_catalog.py::build_strategy_catalog()`, which reads `portfolio/options_strategy.py`'s `MULTI_LEG_STRATEGY_REGISTRY` (43 static entries) - computed fresh on every request, same "cheap local read, not worth caching" precedent as `/api/assets-status`. Deliberately NOT part of `/api/state`'s `state.json` passthrough, since the catalog is useful independent of whether any Lean process has ever run - backs the webui's `/options-strategy` page.
- `GET /api/evaluation` wraps `evaluation_state.py::build_evaluation_state()`, reading `ml/evaluation/*.json` (`aq evaluate`'s rank-book/capacity/stress/ablation reports) and the newest walk-forward summary — each section degrades independently to `{"status": "not_evaluated"}` rather than raising, so a partial evaluation run still returns something useful. `/api/state` and `/api/grafana/retraining-status` needed no route changes for the kill-switch/reconciliation/auto-rollback fields below — both already pass `state.json`/`retraining_status.json` through as-is, and `main.py`/`retraining/status_export.py` write the new keys directly.
