...
Mirrors risk/manual_override.py's read pattern: a single-key, fresh
(never-cached) config.json reader so a long-running paper/live process can
pick up a config edit at the next session rollover without a restart. Also
provides the one new Postgres query paper_readiness_report.py needs -
observation-mode experience_events, which
...
def read_paper_trading_config(config_path: Path) -> dict:
    """Fresh read of phase_v2.paper_trading, no caching. Returns {} if the
    file or key is absent - never raises."""
