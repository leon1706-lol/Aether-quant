...
|---|---|---|
| `requirements/requirements.txt` | local dev, `train.py`, **and** the single consolidated `Dockerfile` (`aether-quant-engine` image — app + every worker) | Full stack: `torch`, `numpy`, `pandas`, `scikit-learn`, `joblib`, `ibapi`, `flask`, `fastapi`, `uvicorn`, `aiofiles`, `redis`, `psycopg[binary]`, and (Phase 4.8) `lean` — the Lean CLI itself, needed at the image level so `retraining-worker`'s `retraining/lean_backtest.py::run_lean_backtest()` can find and invoke it (that service's `docker-compose.yml` entry also mounts the host Docker socket, since `lean backtest` itself launches a separate Docker container to actually run) |
| `requirements/requirements-dev.txt` | local dev only | `lean` (the Lean CLI — correct PyPI name; `lean-cli`/`qcalgorithm` do not exist on PyPI, see this file's own comment), `pytest`, `black`, `fakeredis`, plus `psycopg[binary]` for test imports (connections are mocked in tests), plus `yfinance` (V2-19.5, `data_pipeline/yfinance_backfill.py` — an offline script, never bundled into any container), plus `matplotlib` (`generate_backtest_report.py`, called by `aq backtest` to regenerate README.md's Backtest Results chart) |
