...
# ---------------------------------------------------------------------------
# Kill-switch manual override (V5.1 Phase 6) - same shape as the trade-lock
# override above, independent key, both must be able to coexist untouched.
# ---------------------------------------------------------------------------


def test_kill_switch_read_returns_none_when_key_absent(tmp_path):
    config_path = tmp_path / "config.json"
    _write_config(config_path)
...
def test_kill_switch_read_returns_none_when_config_file_missing(tmp_path):
    assert read_kill_switch_manual_override(tmp_path / "does_not_exist.json") is None


def test_kill_switch_write_true_then_read_round_trips(tmp_path):
    config_path = tmp_path / "config.json"
    _write_config(config_path)
...
def test_kill_switch_write_false_then_read_round_trips(tmp_path):
    config_path = tmp_path / "config.json"
    _write_config(config_path)
...
def test_kill_switch_write_none_removes_the_key(tmp_path):
    config_path = tmp_path / "config.json"
    _write_config(config_path)
    write_kill_switch_manual_override(True, config_path)
    ...
    assert read_kill_switch_manual_override(config_path) is None
    config = json.loads(config_path.read_text(encoding="utf-8"))
    assert "kill_switch_manual_override" not in config["phase_v2"]["risk"]


def test_kill_switch_override_and_trade_lock_override_coexist_independently(tmp_path):
    config_path = tmp_path / "config.json"
    _write_config(config_path)
    ...
    config = json.loads(config_path.read_text(encoding="utf-8"))
    assert config["phase_v2"]["risk"]["manual_trade_lock_override"] is True
    assert config["phase_v2"]["risk"]["kill_switch_manual_override"] is False


def test_kill_switch_write_creates_missing_phase_v2_and_risk_blocks(tmp_path):
    config_path = tmp_path / "config.json"
    config_path.write_text(json.dumps({"name": "Aether Quant"}), encoding="utf-8")
