...
def _count_real_kill_switch_trips() -> int | None:
    """V5.3.1 (development/Problems.md #91/#97): always returns None now -
    this used to count `kill_switch_tripped` occurrences in the real
    backtest's Lean text log, but that was never a real measurement.
    main.py's actual trip-audit event (`self._audit_queue.push(...
    "kill_switch_tripped"...)`) is pushed ONLY to a Redis Stream
    (audit/redis_queue.py::AuditQueue), never to the text log, and
    AuditQueue.push() fails silently with no Redis reachable - which a
    standalone `lean backtest .` run never has. So the old count was
    ALWAYS 0 regardless of how many real trips occurred, not a genuine
    "0 trips" finding (V5.2.10's Changelog entry stated this as if it
    were real - that was wrong, corrected here). No cheap, reliable
    offline proxy exists either: `trade_lock_active` transitions are only
    durably logged per-bar into book_history.jsonl's
    `book_member_decisions`, itself gated to rebalance bars and book-
    selected symbols only - too sparse to reconstruct true rising-edge
    trip counts. Kept as a function (not deleted) so
    _build_other_metrics_markdown() has one clear place to render an
    honest "not measurable from a standalone backtest" caveat instead."""
    return None
...
def _build_other_metrics_markdown(
    ...
    lines.append("")
    # V5.3.1 (development/Problems.md #91/#97) - _count_real_kill_switch_trips()
    # always returns None now (see its own docstring for why the old
    # count was never a real measurement) - render an explicit caveat
    # instead of a bare "—" that could be misread as "zero trips".
    if kill_switch_replay and kill_switch_replay.get("summary"):
        ...
        lines.append("|---|---|---|")
        lines.append(
            f"| Real Lean backtest{f' ({lean_window})' if lean_window else ''} | "
            "_not measurable from a standalone backtest (see Disclaimer)_ | — |"
        )
        lines.append(
        ...
        lines.append("_Not yet available - run `aq evaluate --replay-kill-switch`._")
