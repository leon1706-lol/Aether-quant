...
- **A first real `lean backtest .` has now been run** (2019-01-01 → 2021-03-31, results in [Backtest Results](#backtest-results)) — the core equity/crypto/bond long path (inference → gating → regime → liquidity → topology → sizing → order application) executed end-to-end in a real Lean runtime, no longer only unit-tested. **It surfaced a real strategy-behavior finding, not a crash:** only ~14 positions ever opened and none closed (buy-and-hold, mostly bond ETFs), because the model's `probability_up` outputs cluster tightly around the decision threshold (~0.46–0.49) while the buy line sits at 0.50 and the sell line at 0.42 — so entries are rare and exits almost never trigger. See `development/Problems.md` #41 for the full root-cause analysis and the threshold-calibration lever toward a higher, statistically meaningful trade count. **Still NOT exercised by any real backtest** (all config-gated off for that run, so their own verification lists remain fully open): **real limit orders** (`phase_v2.limit_orders`) rest on unverified Lean API naming assumptions (PascalCase method/enum casing, `OrderStatus.Filled`, `on_order_event` dispatch — see `execution/README.md`'s "Real limit orders" section), and the **2-leg vertical options spread** (`phase_v2.options_risk.spread_strategy: "vertical"`, `development/Problems.md` #38 — single-leg remains the default; straddles/strangles/iron condors/butterflies are still not implemented at all) has the largest verification list of any feature — zero prior combo-order usage in this codebase (see `risk/README.md`'s verification list).
