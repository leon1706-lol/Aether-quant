...
**Problem:** #102 found `min_rank_confidence_spread` (same-day raw-score dispersion) can't distinguish "the model differentiates symbols today" from "that differentiation is directionally correct" — the recalibrated threshold traded through real, known no-skill/negative-IC eras because dispersion looked normal even when skill wasn't there. #102's own recommendation: "find a smarter calibration that accounts for per-era IC stability, not just raw-score dispersion."

**Fix:** a new, independent second veto — `portfolio/rolling_ic_gate.py::evaluate_rolling_ic_gate()` — gates book engagement on the TRAILING REALIZED rank-IC (was the model actually right, recently), computed from a live in-memory buffer of past predictions resolved against their own realized 20-day forward return, entirely separate from `min_rank_confidence_spread`. New `evaluation/rank_ic_core.py` extracts the correlation/resolution math out of `train.py`/`performance/rank_ic_monitor.py` into a genuinely torch-free module so `main.py` can import it without risking the isolator-timeout class of failure in #16 (train.py imports torch/sklearn at module scope; `performance/__init__.py` transitively imports train.py). Two new offline tools mirror this codebase's existing calibration/replay pattern: `aq evaluate --calibrate-rolling-ic-floor` (derives the floor from the real walk-forward rolling-IC distribution, never guessed) and `aq evaluate --replay-rolling-ic-gate` (day-by-day, no-lookahead replay with a per-era breakdown against the three known bad eras, the critical pre-`main.py` checkpoint). `portfolio/book_construction.py` gained two additive, `None`-default parameters (`rolling_ic_gate_result`, `veto_reason_out`) rather than a return-type change — `build_rank_based_book()` has 6+ real callers beyond `main.py`, and a tuple return would have rippled through every one of them for a diagnostic-only need. `build_book_history_record()` gained a `gate_veto_reason` field, and `main.py`'s book-history write condition was fixed to log on every genuine rebalance bar even when the book comes back empty (previously a vetoed bar left zero trace in the log at all, true since #91/V5.2.4 for the *existing* dispersion gate too).

**A real bug found and fixed mid-round, via the gate's own decision checkpoint:** the first real calibration+replay pass did NOT show the expected pattern — era_2 and era_3 (2 of the 3 known bad eras) showed 0% disengagement even at a raised floor, while the calibrated distribution's minimum was an implausible -0.9999999999999999. Root cause: origin dates with only 2-3 resolved names are mathematically FORCED to a Spearman correlation of exactly ±1 regardless of real skill (two points always fit a line) — these thin-universe readings were getting averaged into `mean_ic` with the same weight as a 100-name date, dominating the low tail of the distribution. Fixed with a new `min_names_per_date` parameter threaded through `rank_ic_from_arrays()`/`aggregate_rank_ic_observations()`/`compute_rolling_ic_state()` (default 2, preserving every existing caller's behavior exactly; the offline calibration/replay tools and the live gate's own config default to 10). This surfaced a SECOND bug: with `min_names_per_date=10` filtering the trailing window, `num_resolved_dates` structurally plateaus at ~22-24 out of a 40-day window across the *entire* dataset (checked start, middle, and end - not a warmup artifact) — the shipped `min_resolved_dates_required=40` could then never be satisfied, so the gate always fell back to "insufficient history" regardless of the real IC reading. Lowered to 20 (safely below the observed structural ceiling).

**Result, after both fixes — real, measured:** calibrated `min_rolling_mean_ic` at p25 of the corrected distribution (+0.020, `min_names_per_date=10`) replayed against `min_resolved_dates_required=20`: 20/82 rebalance dates disengaged overall (24.4%), cleanly concentrated — era_0 (the good era) **0% disengaged**, era_1 (Apr-Sep 2019) **37.5%**, era_2 (Dec 2019-Mar 2020) **22.2%**, era_3 (Jun-Sep 2020) **22.2%**. Spot-checked era_3's raw per-date readings directly: mostly strongly positive (0.19-0.63) with a real, brief no-skill/negative dip in mid-July 2020 (0.005, then -0.091) that the gate correctly and narrowly catches, rather than a false uniform trip.

**Verification:** new `tests/test_rank_ic_core.py` (13 tests, including the moved functions and the new `min_names_per_date` filter directly), `tests/test_rolling_ic_gate.py` (14 tests), `tests/test_rolling_ic_gate_calibration.py` (7 tests, including a no-lookahead prefix-invariance test), `tests/test_rolling_ic_gate_replay.py` (8 tests, no-lookahead correctness is the headline property tested), `tests/test_portfolio_book_construction.py` extended (+9 tests, including a byte-identical-when-`None` regression guard and a deterministic-veto-ordering test). Full suite: 2688 passed, 0 failures (11 pre-existing Docker-unavailable errors, unrelated). `--calibrate-rolling-ic-floor`/`--replay-rolling-ic-gate` both run for real against `ml/datasets/backtest_dataset.csv` (not simulated/guessed). Fresh `--reconcile-book-history --replay-hysteresis --reconcile-all-runs` against all 9 existing real backtests confirmed the new optional parameters didn't change the *existing* reconciliation story (no errors, all 9 runs reconciled cleanly). `config.json`'s `phase_v2.rolling_ic_gate` block ships with `enabled: false` — this round did not flip it live or run a real Lean backtest.

**Follow-ups:** the user's own manual `aq backtest` is the only thing that can verify this actually improves live Sharpe over #102's -1.798 baseline — same discipline #102 itself demonstrated (the offline story looked right for the confidence-spread recalibration too, and a real backtest still showed a regression). Before that run: `aq config set phase_v2.rolling_ic_gate.min_rolling_mean_ic 0.02` then `aq config set phase_v2.rolling_ic_gate.enabled true`. After the run: check the new `gate_veto_reason` field in the real `book_history.jsonl` for `"rolling_ic_below_floor"` during the bad eras specifically (not just `"min_rank_confidence_spread_below_floor"`, which would mean the old gate is still doing all the work). Workstream B (a feature-level investigation into XOM's still-unresolved divergence, see #91/#100) was scoped for this round but not started — independent, can run separately.

**Update (2026-08-19 real backtest — written up in V5.3.5.3):** the prescribed run happened (acktests/2026-08-19_13-33-56, both q config set commands applied first, gate live at floor 0.02). **Headline result: Sharpe -1.798 -> -1.832 — the rolling-IC gate did NOT recover the regression** (Net Profit -5.51%->-5.888%, Drawdown 6.90%->6.400%). But the mechanism itself works exactly as built: orders fell 695->462 (-33%), fees $880->$596.54, and the new gate_veto_reason field answers #103's own verification question directly — across the cumulative log the new veto fired 106x vs the old spread gate's 21x (plus 260 pre-existing 	hin_universe_or_zero_top_n), i.e. olling_ic_below_floor genuinely does most of the disengagement work now; it is not a no-op passenger. Interpretation, honestly stated: the gate successfully refuses low-skill stretches, but what it still ALLOWS through isn't profitable either — engagement quality outside the bad eras is the binding constraint, and the pre-V5.3.3 -1.034 baseline remains unrecovered. Per the user's explicit decision this round, the gate stays enabled at 0.02 and min_rank_confidence_spread stays at 0.2831 unchanged, so the V5.3.5.3 diagnostic round measures exactly this configuration. Workstream B itself shipped in V5.3.5.3 (see Changelog): uild_book_history_record()'s allowlist-bounded eature_snapshot field, the two phase_v2.diagnostics.book_history.include_feature_snapshot/eature_snapshot_symbols flags, and q evaluate --reconcile-features --symbol XOM; XOM's actual feature-level verdict awaits the diagnostic backtest that collects snapshot data.
...
**Problem:** commit c2ed98c's "endless loop found in backtest log" is Lean's embedded-interpreter teardown timing out: after PythonInitializer.Shutdown(): start/calling engine shutdown..., ~10s elapse, then Isolator.ExecuteWithTimeLimit(): Execution Security Error: Operation timed out - 0.1666... minutes max. Check for recursive loops. (Isolator.cs:179) and Program.Exit(): Failed to shutdown python System.TimeoutException (Launcher/Program.cs:145). Classic signature of CPython finalization blocking on a still-alive non-daemon thread or native handle while Lean's isolator kills the interpreter at its 10s limit.

**Differential analysis (all local backtest logs checked):**
- **Chronic, not new:** 72 of 78 runs that reached teardown hang identically, back to the earliest log on disk (2026-05-07) — spanning many code versions, so nothing recent (not V5.3.x, not the rolling-IC gate) introduced it.
- **Never affects results:** the hang always occurs after statistics/JSONs/order-events are fully written; impact is limited to an ERROR line, a failed exit code, and ~10s of wall clock.
- **Normal completion vs error termination is the discriminator:** both genuinely-clean completed runs (2026-07-06_17-35-38, 2026-07-19_16-48-18) terminated via a mid-run Python runtime error or Stopped state; essentially every run whose Python execution ran to normal completion hangs. Something built up during normal bar processing survives to block interpreter finalization.
- **Ruled out:** run duration/window size (a 4.53s run hung; a 14.53s run exited cleanly); the inference parallelism pool (#70's explicit pool.shutdown() exists, and the pool ships default-off = None anyway); audit/experience queues (no threads / daemon=True drain thread); matplotlib/pyplot use in main.py (none); cpp_inference_ext usage in the hot path (none).

**Fix (this round):** per this codebase's own discipline, no speculative fix without a named culprit. Instead, main.py::on_end_of_algorithm() gained a zero-risk shutdown-probe: Debug line dumping every still-alive thread (name + daemon flag) right before teardown begins, so the next real backtest's log shows exactly what survives normal completion and blocks finalization.

**Verification:** py_compile only so far (no unit test can exercise Lean's embedded-interpreter teardown). Decisive evidence is the next real backtest: its log should carry the shutdown-probe: line immediately before the timeout (or, if the run exits cleanly with the probe present, that narrows the trigger too).

**Follow-ups:** read shutdown-probe: alive_threads=[...] from the next run's log; once the surviving resource is named, add targeted cleanup (explicit close/shutdown of the owning library object) and re-verify with a subsequent run's clean PythonInitializer.Shutdown(): ended.
