...
**Next step:** one more real Lean backtest with both fixes applied. If new orders still stop around the same point, the spread-gate hypothesis is disproven and the actual cause needs fresh investigation (candidates not yet ruled out: the eligible-candidate pool itself shrinking, a `trading_eligible`/feature-readiness interaction specific to the January warmup period, or something in how `resolve_rank_signal_policy()`'s heads resolve during that specific window). If trading resumes and continues, the two fixes were sufficient.
