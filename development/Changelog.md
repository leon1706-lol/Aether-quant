...
**Summary:** #102 found `min_rank_confidence_spread` (same-day raw-score dispersion) can't tell "the model differentiates symbols today" apart from "that differentiation is directionally correct," and recommended a calibration that accounts for per-era IC stability instead. This round builds and offline-verifies exactly that: an independent, second book-engagement veto based on trailing REALIZED rank-IC, plus the offline calibration/replay tooling to derive and validate its threshold from real data before ever touching `main.py`. Ships with `phase_v2.rolling_ic_gate.enabled: false` — a real Lean backtest is still needed to confirm the live effect, same discipline #102 itself established.

**Shipped:**
- New `evaluation/rank_ic_core.py` — extracts `train.py`/`performance/rank_ic_monitor.py`'s rank-IC correlation and realized-outcome-resolution math into a genuinely torch-free module, so `main.py` can import it directly without risking the isolator-timeout failure class documented in `development/Problems.md` #16. The two original modules now re-export it unchanged — behavior-preservation proven by their own existing test suites passing unmodified.
- New `portfolio/rolling_ic_gate.py` (`compute_rolling_ic_state()`, `evaluate_rolling_ic_gate()`) — a pure, independently-testable second veto, deliberately separate from `book_construction.py`'s same-bar dispersion check and from `risk/kill_switch.py`'s dormant, differently-scoped `min_live_rank_ic` condition.
- `portfolio/book_construction.py`: `_select_book_group()`/`build_rank_based_book()` gained two additive, `None`-default parameters (`rolling_ic_gate_result`, `veto_reason_out`) instead of a return-type change — `build_rank_based_book()` has 6+ real callers beyond `main.py`, and a tuple return would have rippled through every one for a diagnostic-only need. `build_book_history_record()` gained a `gate_veto_reason` field. `main.py`'s book-history write condition fixed to log on every genuine rebalance bar, even a vetoed one that comes back empty (previously left zero trace in the log — true since #91/V5.2.4 for the *existing* dispersion gate too).
- New `evaluation/rolling_ic_gate_calibration.py` (`aq evaluate --calibrate-rolling-ic-floor`) and `evaluation/rolling_ic_gate_replay.py` (`aq evaluate --replay-rolling-ic-gate`, with a per-era breakdown against the three known bad eras) — mirror this codebase's existing `--calibrate-book-spread`/`--replay-kill-switch` discipline: derive the threshold from data, verify the walk-forward pattern offline, before the live path is ever touched.
- `main.py`: new live in-memory event buffer (Pass 1c, pre-normalization `raw_rank_score`, same precedent as `_kill_switch_return_history`), gate evaluation wired in right before `build_rank_based_book()`, new `phase_v2.rolling_ic_gate` config block (`enabled: false`).

**A real bug found and fixed via the gate's own decision checkpoint, not assumed away:** the first calibration+replay run against real data did not show the expected per-era pattern (2 of 3 known bad eras showed 0% disengagement). Root-caused to origin dates with only 2-3 resolved names being mathematically forced to a ±1 IC reading regardless of real skill, dominating the calibrated distribution's low tail. Fixed with a new `min_names_per_date` parameter (default 2, preserving every existing caller; the new gate's own tools/config default to 10) — which then surfaced a second, related bug: the resulting stricter per-date filter meant `min_resolved_dates_required=40` could structurally never be satisfied, permanently masking the gate as "insufficient history." Lowered to 20. After both fixes, the replay shows a clean, honest pattern: era_0 (good) 0% disengaged, and all three known bad eras 22-37.5% disengaged, concentrated correctly (verified against the raw per-date readings directly, not just the aggregate).

**Verification:**
- 51 new tests across `test_rank_ic_core.py`, `test_rolling_ic_gate.py`, `test_rolling_ic_gate_calibration.py`, `test_rolling_ic_gate_replay.py`, plus 9 new tests extending `test_portfolio_book_construction.py`. Full suite: 2638 → 2688 passed, 0 failures (11 pre-existing Docker-unavailable errors, unrelated).
- `--calibrate-rolling-ic-floor` and `--replay-rolling-ic-gate` both run for real against `ml/datasets/backtest_dataset.csv` (not simulated) — see `development/Problems.md` #103 for the full numbers.
- Fresh `--reconcile-book-history --replay-hysteresis --reconcile-all-runs` against all 9 existing real backtests: no errors, existing reconciliation story unchanged by the new optional parameters.
- No real Lean backtest run this round. `enabled` ships `false` — the user's own manual `aq backtest` is the next step, per this codebase's established real-verification discipline.

See `development/Problems.md` #103 for full evidence and scope notes.
...
**Summary:** three threads in one small round, none touching trading behavior: (1) Workstream B from the V5.3.5 plan — the XOM feature-level investigation's plumbing, scoped in #91/#100 lineage — is now fully built and tested; (2) the previously undocumented 2026-08-19 real backtest (acktests/2026-08-19_13-33-56, the rolling-IC gate's first live test) is now written up in development/Problems.md #103; (3) the "endless loop" from commit c2ed98c is root-caused as far as the available evidence allows and instrumented for a deterministic name-the-culprit next run (development/Problems.md #104).

**Workstream B — what shipped:**
- evaluation/feature_reconciliation.py::reconcile_feature_snapshot() + summarize_feature_reconciliation() (started last round but left untracked/unimported) adopted; one real bug found by the new tests on adoption day itself: the summary aggregator counted whole per-feature delta dicts as if they were feature names, crashing any summary built from real input — fixed.
- portfolio/book_construction.py::build_book_history_record() gained an additive, None-default eature_snapshot field (same byte-identical-when-None convention as every optional field before it), defensive-copied so the record can never alias main.py's live pass1_state.
- main.py: two new flags under phase_v2.diagnostics.book_history — include_feature_snapshot (default false) and eature_snapshot_symbols allowlist — read at Initialize; the book-history write site captures pass1_state[...]["feature_payload"]["base_features"] for allowlisted tickers only (ticker-matched via sset_lookup, not raw Lean symbol strings). Allowlist-bounded by design: an unbounded snapshot would serialize ~77 symbols x dozens of floats every rebalance bar for a diagnostic that only ever needs a handful of tickers.
- New CLI: q evaluate --reconcile-features --symbol XOM [--book-history-path <path>] — diffs each logged snapshot against the same (ticker, date) row of ull_dataset.csv, per-feature deltas sorted worst-first, aggregated into ml/evaluation/feature_reconciliation.json. Deliberately much lighter than its sibling --reconcile-book-history: no model artifacts, no re-inference — a pure log-vs-dataset diff. Reuses the existing cumulative-log discipline (run-segmented, defaults to the most recent run, --reconcile-run-index/--reconcile-all-runs honored). Not in --all, investigation-only.

**Tests:** new 	ests/test_feature_reconciliation.py (14 tests: tolerance AND-condition semantics, NaN/None degrade-to-None, only-in-* informational lists, sort order, empty/disjoint inputs, custom tolerances, aggregation/cap/never-compared logic); 	ests/test_portfolio_book_construction.py extended (+3: None-omits-key regression guard, populated-snapshot emission, copy-not-alias); 	ests/test_aq_cli.py extended (+3 end-to-end: diverging-fixture reconciliation with JSON artifact equality, graceful zero-snapshot degradation, missing-symbol error). Registered in _SUBSYSTEM_TEST_FILES under evaluation. Full suite: 2688 -> 2708 passed, 0 failures. README badge refreshed to 2708/2708.

**Rolling-IC gate's first live test (backfilled from 2026-08-19, see #103):** Sharpe -1.832 vs the -1.798 baseline it was meant to beat — no recovery — while orders fell 695->462 and olling_ic_below_floor out-vetoed the old spread gate 106:21. The gate works; what it still lets through just isn't profitable either. Config left exactly as-is this round per user decision.

**Shutdown hang (#104):** verified chronic (72/78 teardown-reaching runs back to May 2026), always post-results (zero impact on output), and strictly correlated with NORMAL completion — both clean exits were mid-run Python error terminations. Inference pool (#70) exonerated; no other non-daemon thread creators exist in the live path. Shipped a shutdown-probe: Debug dump of still-alive threads in on_end_of_algorithm(); next real backtest names the culprit.

**Verification:** full suite green (2708 passed, excludes the lean_backtest marker); manual dry-run of --reconcile-features --symbol XOM against the real cumulative isualization/book_history.jsonl (10 run segments detected, 0 snapshots yet — correct, the producing flags are new) degrades cleanly with an actionable hint, exit 0. No real Lean backtest run this round — the user's manual diagnostic run (snapshot flags now ON for XOM) is the next step, then q evaluate --reconcile-features --symbol XOM reads its verdict.
