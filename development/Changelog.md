...
**Shipped:**
- Stage 1: `execution/order_gate.py::classify_order_status()` now explicitly classifies `"CancelPending"` as `"pending"` (confirmed via 33 real `order-events.json` files — 23 occurrences, always paired 1:1 with a following `"canceled"`; a precision fix, zero behavior change). `Problems.md` #60/#61 corrected — Forex is fully live and verified (not "zero live tickers" as originally stated), and the bond-feature-schema retrain caveat was already closed by #70, stale doc only.
- Stage 2a: new `evaluation/kill_switch_replay.py` (`replay_kill_switch_over_dataset()`/`summarize_kill_switch_replay()`) — a day-by-day offline replay of the kill-switch + sticky trade-lock state machine against the rank book's own return series, explicitly labeled an approximation (no bypass flags, no `net_edge`/book-selection modeling). New `aq evaluate --replay-kill-switch` flag, not in `--all`.
- Stage 2a: kill-switch sensitivity sweep (`min_rolling_sharpe` × `evaluation_bars`, 20 combinations) — real finding: lockout duration is close to binary under a non-bypassed replay (either a trip never happens, or it locks out ~58-74% of the remaining ~2.2-year dataset regardless of exactly how strict the threshold is beyond that). No config change applied — findings only.
- Stage 2b: new `train.py::compute_gate_friendliness_weight_by_date()` — an optional per-date training-loss weight from the same stateless topology/regime-severity gates the live analyzer uses (liquidity and the portfolio-level kill-switch deliberately excluded — no clean per-row training-dataset equivalent for either). `soft_spearman_loss()`/`listnet_loss()`/`compute_cross_sectional_ranking_loss()`/`compute_combined_multitask_loss()` gain an optional `date_weights` parameter (`None` default, byte-identical to today). Wired into `train_multitask.py`/`train_sequence.py` behind `phase_v2.retraining.{multitask_training,sequence_training}.gate_aware_ranking_weights.enabled` (default `false`); only the training loss is weighted, validation/early-stopping stays unweighted.
- Stage 2c/2d: re-ran `aq evaluate --reconcile-book-history --replay-hysteresis` against the real, isolated V5.2.7 book history. NVDA/GE/WFC/XOM/BA all still recur prominently (NVDA the single most-frequent live-only mismatch, GE the most-frequent offline-only). Checked the one concrete lead from #90's bond-duration-beta bug pattern against `features/cross_asset_sensitivity.py` (the only other feature using the same live rolling-deque style) — ruled out, live and offline use the identical 252-day lookback window. Both threads remain genuinely unexplained.

**Verification:**
- 33 new tests (1 `order_gate`, 14 `kill_switch_replay`, 8 `date_weights` parity in `test_train_ranking_loss.py`, 10 new `test_train_gate_friendliness.py`) — full suite 2523 → 2556 passed, 0 failures. `py_compile` clean on every touched file.
- `aq evaluate --replay-kill-switch` confirmed end-to-end against the real dataset (43 trips, 399/565 locked days at default config).
- No `main.py` changes this round — every new mechanism is evaluation-only or training-only/config-gated-off. A lightweight Codespace smoke-test run (not a full retrain) verifies the training-side addition runs cleanly; whether it should ever become the default is a separate, future, evidence-based decision.
