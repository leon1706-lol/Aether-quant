...
def main() -> int:
    ...
    try:
        ...
        for epoch in range(1, max_epochs + 1):
            ...
            # direction MCC but DEGRADED the rank_20d backtest signal
            # (non-overlapping t-stat 2.90 -> 2.21, confirmed by direct
            # comparison during development) - training longer against a
            # head this model doesn't actually get used for is not obviously
            # correct. Still fixes the untrained-init bug via
            # is_new_best_epoch()'s min_epoch floor below (best_epoch=1 is
            # never eligible), just without changing WHAT is optimized for.
